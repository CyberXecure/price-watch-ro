...
def _ui_unit_from_db_enum(unit) -> str | None:  
    normalized = normalize_comparison_unit(unit)  
    mapping = {  
        "LEI_PER_L": "lei/l",  
        "LEI_PER_KG": "lei/kg",  
        "LEI_PER_BUC": "lei/buc",  
...
def upsert_product_snapshot_and_watchlist(  
    ...
    if payload.watchlist_id is not None:  
        ...
        if forced_watchlist_item_id is not None:  
            row = session.exec(  
                text(  
                    """  
                    SELECT id, watchlist_id, store_product_id, target_price, target_unit, current_status  
                    FROM watchlistitem  
                    WHERE id = :item_id AND watchlist_id = :watchlist_id  
                    """  
                ),  
                params={  
                    "item_id": forced_watchlist_item_id,  
            ...
            effective_target_unit_db = normalized_target_unit_db or normalized_comparison_unit  
            ...
            session.exec(  
                ...
                params={  
                    ...
                    "notify_best_buy": 1 if payload.notify_best_buy else 0,  
                    "notify_high_price": 1 if payload.notify_high_price else 0,  
                    "item_id": forced_watchlist_item_id,  
                    "watchlist_id": payload.watchlist_id,  
                },  
            )  
            ...
            watchlist_item_row = session.exec(  
                text(  
                    """  
                    SELECT id, watchlist_id, store_product_id, target_price, target_unit, current_status  
                    FROM watchlistitem  
            ...
            if watchlist_item_row is not None and hasattr(watchlist_item_row, "_mapping"):  
                row_map = dict(watchlist_item_row._mapping)  
                watchlist_item_id_value = row_map.get("id", forced_watchlist_item_id)  
                watchlist_item_store_product_id_value = row_map.get("store_product_id", product.id)  
                current_status_value = row_map.get("current_status")  
            else:  
                watchlist_item_id_value = forced_watchlist_item_id  
                watchlist_item_store_product_id_value = product.id  
                current_status_value = None  
            ...
            if watchlist_item:  
                ...
                watchlist_item = WatchlistItem(  
                    ...
                    store_product_id=product.id,  
                    target_price=payload.target_price,  
                    target_unit=normalized_target_unit_db or normalized_comparison_unit or "TOTAL",  
    ...
    return {  
        ...
        "comparison_price": comparison_price,  
        "comparison_unit": _ui_unit_from_db_enum(normalized_comparison_unit),  
